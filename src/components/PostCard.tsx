...
export function PostCard({ post, onEdit, onDelete, index = 0 }: PostCardProps) {
  ...
  return (
    <motion.article 
      ...
      className="group bg-white rounded-2xl overflow-hidden mb-6 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.08)] hover:shadow-[0_12px_40px_-8px_rgba(118,149,236,0.25)] border border-[#f0f0f0] hover:border-primary-blue/30 transition-all duration-500"
    >
        ...
        <div className="flex justify-between items-center mb-5 pb-5 border-b border-neutral-100">
          <div className="flex items-center gap-3">
            <motion.div 
              ...
              className="w-11 h-11 rounded-xl bg-linear-to-br from-primary-blue to-primary-dark flex items-center justify-center text-white font-bold text-sm shadow-md shadow-primary-blue/20"
            >
          ...
          <div className="flex items-center gap-2 text-xs text-neutral-500 bg-neutral-100 px-3 py-2 rounded-full border border-[#f0f0f0]">
            <Clock className="w-3.5 h-3.5" />
            <span className="font-medium">{formattedDate}</span>
          </div>
        </div>
        
        <p className="text-neutral-600 text-base leading-relaxed whitespace-pre-wrap break-words">
          {post.content}
        </p>
      </div>
    </motion.article>
